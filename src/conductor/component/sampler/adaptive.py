...
import numpy as np
from conductor.component.sampler._base import Sampler

import logging
logger = logging.getLogger("conductor.component.sampler.adaptive")
...
class AdaptiveSampler(Sampler):
    ...
    def __repr__(self):
        return Sampler.__repr__(self) + ":" + AdaptiveSampler._name

    def __init__(self, plan_size, configs=None):
        Sampler.__init__(self, "adaptive", configs=configs, child_default_configs={
            "sample_cnt_step": 8
        })
        self.next_update = plan_size
        self.max_sample_cnt = plan_size   
        self.sample_cnt_step = self.config["sample_cnt_step"]
    ...
    def sample(self, samples, dims, threshold=2.5, valid_dims=2):
        ...
        cardinal_dims_samples = [
            np.array(sample)[cardinal_dims] for sample in samples]
        ...
        reduced_samples = reduce_samples(
            samples, centroids, cluster, dims, cardinal_dims)
...
def clusterize(points, k, max_iter=100):
    ...
    while i < max_iter or split == True and new_assignment != old_assignment:
        ...
        for p in range(len(points)):
            distances = [distance(points[p], centroids[c])
                         for c in range(len(centroids))]
            new_assignment[p] = np.argmin(distances)

        for c in range(len(centroids)):
            members = [points[p]
                       for p in range(len(points)) if new_assignment[p] == c]
            if members:
    ...
    for d in range(len(dims)):
        if d in cardinal_dims:
            ...
            if reduction == 'sample':
                ...
                sample.append(
                    max(set(sample_dims[d]), key=sample_dims[d].count))
...
def reduce_samples(samples, centroids, cluster, dims, cardinal_dims):
    ...
    for c in range(len(centroids)):
        ...
        while unique is False:
            new_sample = synthesize_sample(
                centroids[c], sample_dims, dims, cardinal_dims)
