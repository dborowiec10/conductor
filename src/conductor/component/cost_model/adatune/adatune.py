...
class RFModel(CostModel):
    def __init__(self, task, feature_type="itervar", num_threads=None, log_interval=25, upper_model=None):
        super().__init__()
        self.task = task
    ...
    def _get_feature(self, indexes):
        ...
        # free feature cache
        if self.feature_cache.size(self.feature_type) >= 100000:
            self.feature_cache.clear(self.feature_type)

        fea_cache = self.feature_cache.get(self.feature_type)
    ...
    def fit(self, xs, ys, plan_size):
        ...
        # transfer into corresbonding x_list of feature_type
    ...
    def spawn_base_model(self):
        return RFModel(self.task, self.feature_type, self.loss_type, self.num_threads, self.log_interval, self)
...
#         cost_model = RFModel(task, feature_type=feature_type)
#         if optimizer == 'sa':
