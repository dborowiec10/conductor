...
import logging
logger = logging.getLogger("conductor.component.builder.parallel")

class ParallelBuilder(Builder):
    _name = "paralell"
    
    def __repr__(self):
        return Builder.__repr__(self) + ":" + ParallelBuilder._name

    def __init__(self, configs=None, profilers_specs=[]):
        Builder.__init__(self, "parallel", configs=configs, child_default_configs=Configurable.merge_configs({
            "timeout": 10,
            "n_parallel": None,
            "build_func": "default"
        ...
        self.max_n_parallel = psutil.cpu_count(logical=True)
        if n_parallel == None:
            self.n_parallel = self.max_n_parallel
        else:
            self.n_parallel = n_parallel

    def set_n_parallel(self, npar):
        if npar > self.max_n_parallel:
            self.n_parallel = self.max_n_parallel
        else:
