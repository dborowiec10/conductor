...
logger = logging.getLogger("conductor.component.measurer.doppler_learn")

class DopplerLearn(DopplerBaseMeasurer):
    _name = "doppler_learn"
    
    def __repr__(self):
        return DopplerBaseMeasurer.__repr__(self) + ":" + DopplerLearn._name

    def __init__(self, builder, runner, configs={}, child_default_configs={}):
        DopplerBaseMeasurer.__init__(self, "doppler_learn", builder, runner, configs=configs, child_default_configs=Configurable.merge_configs({
            "max_dop": 64,
            "window_deltas": 2,
            "window_times": 2,
            ...
            "opt_propose_avg_time_error_split": [0.4, 0.6],
    ...
    def optimizers_propose(self):
        ...
        split = self.config["opt_propose_avg_time_error_split"]

        avg = math.floor((split[0] * mt_prop) + (split[1] * cne_prop))
        et = time.time()
        logger.info("TASK[%s] optimizers_proposed_dops: TIME[%d], DELTA[%d] ... avg(%d), in %f seconds", self.task_idx, mt_prop, cne_prop, avg,  et - st)

        if avg == self.task_states[str(self.task_idx)]["last_proposed_dop"]:
        ...
        if self.task_states[str(self.task_idx)]["last_it_failed"] > 0:
            avg = avg - int(math.ceil(avg * self.task_states[str(self.task_idx)]["last_it_failed"]))
            logger.info("TASK[%s] optimizers_proposed_dops: reducing dop by %f as candidates failed due to dop during last run: current proposed: %d", self.task_idx, self.task_states[str(self.task_idx)]["last_it_failed"], avg)
        return avg
