...
logger = logging.getLogger("conductor.compiler.tensor_program_executor")

class TensorProgramExecutor(Task, Configurable, Profilable):
    _name = "tensor_program_executor"
    
    def __repr__(self):
        return Task.__repr__(self) + ":" + TensorProgramExecutor._name
    ...
    def __init__(self, spec, task_results_path, models_path, tensor_programs_path):
        ...
        Configurable.__init__(self, "executor", ["tensor_program"], self.spec.configurations, Configurable.merge_configs({
            "timeout": 20
        }, {}, override_first=True))
    ...
    def run(self, inputs, idx):
        ...
        stat = "Executing Tensor Programs: ["
        ...
        while left_to_do > 0:
            tp, opts, evaluator = pairs[glob_idx]
            ...
            Experiment.current.add_tp_and_opt(
                {
                    ...
                    "is_topi": tp.is_topi,
                    "instruction_count": tp.instr_count,
                    "implementations_type": tp.implementations_type,
                    "implementations_path": tp.implementations_path,
                },
                {
